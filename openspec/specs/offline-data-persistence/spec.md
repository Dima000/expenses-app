...
Durable local persistence of Firestore data in the web client so that reads and writes keep working across cold starts and offline periods: subscriptions are served from a local cache instead of returning empty results, and writes made while offline are queued and synchronised automatically once connectivity is restored.
