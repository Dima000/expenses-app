# data-source-abstraction Specification

## Purpose

The architectural guarantee that spending and category reads/writes flow through a single swappable `DataSource` interface rather than components calling Firestore directly, and that behavior is unchanged through that interface. This is what lets a future non-Firestore implementation (e.g. a demo-mode data source) be substituted with no call-site changes.
